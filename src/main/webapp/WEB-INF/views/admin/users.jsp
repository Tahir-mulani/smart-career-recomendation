                        ...
                        <% if (users != null && !users.isEmpty()) { %>
                            <% for (User user : users) { %>
                                <tr>
                                    ...
                                    <td><%= user.getSkills() != null && user.getSkills().length() > 20 ? user.getSkills().substring(0, 20) + "..." : (user.getSkills() != null ? user.getSkills() : "N/A") %></td>
                                    <td><%= user.getRegistrationDate() != null ? user.getRegistrationDate().toString() : "N/A" %></td>
                                    <td><span class="admin-badge active">Active</span></td>
                                    <td>
                                        <a href="/admin/users/<%= user.getId() %>" class="admin-action-btn view" title="View"><i class="fas fa-eye"></i></a>
                                        <form action="/api/admin/users/<%= user.getId() %>/delete" method="post" style="display: inline;" onsubmit="return confirm('Are you sure you want to delete this user?');">
                                            <button type="submit" class="admin-action-btn delete" title="Delete"><i class="fas fa-trash"></i></button>
                                        </form>
                                    </td>
                                </tr>
                            <% } %>
