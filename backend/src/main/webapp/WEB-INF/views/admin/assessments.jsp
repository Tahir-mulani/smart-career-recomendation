...
<body class="admin-body">
		...
		<aside class="admin-sidebar" id="sidebar">
			...
			<nav class="admin-sidebar-nav">
				<ul>
					...
					<li><a href="/admin/assessments" class="active"><i class="fas fa-file-alt"></i> Assessments</a></li>
		...
		<main class="admin-main">
			...
			<div class="admin-content">
				...
				<div class="admin-form">
					<h3><i class="fas fa-plus-circle"></i> Create New Assessment Test</h3>
					<form action="/api/admin/create-assessment" method="post">
						<div class="admin-form-row">
							<div class="admin-form-group">
								<label for="testName"><i class="fas fa-heading"></i> Assessment Test Title</label>
								<input type="text" id="testName" name="testName" placeholder="e.g. Full-Stack Web Development Assessment" required>
							</div>
							<div class="admin-form-group">
								<label for="duration"><i class="fas fa-clock"></i> Duration (Minutes)</label>
								<input type="number" id="duration" name="duration" placeholder="e.g. 45" required>
							</div>
							<div class="admin-form-group">
								<label for="totalMarks"><i class="fas fa-award"></i> Total Marks</label>
								<input type="number" id="totalMarks" name="totalMarks" placeholder="e.g. 100" required>
							</div>
						</div>
				...
				<!-- Table of Existing Assessments -->
				<div class="admin-table-container">
					<div class="admin-table-header">
						<h3><i class="fas fa-list-alt"></i> Existing Assessment Modules</h3>
					</div>
					<table class="admin-table" id="assessmentsTable">
						...
						<tbody>
							...
							Map<Long, Integer> questionCounts = (Map<Long, Integer>) request.getAttribute("questionCounts");
							if (assessments != null && !assessments.isEmpty()) {
								for (Assessment a : assessments) {
									int count = (questionCounts != null && questionCounts.get(a.getId()) != null) ? questionCounts.get(a.getId()) : 0;
							%>
							<tr>
								<td>#<%= a.getId() %></td>
								<td><strong><%= a.getTestName() %></strong></td>
								<td><%= a.getDuration() %> Mins</td>
								<td><%= a.getTotalMarks() %> Marks</td>
								<td><span style="color: var(--primary-cyan); font-weight: 600;"><%= count %> Questions</span></td>
								<td>
									<button class="admin-action-btn edit" onclick="openEditModal(<%= a.getId() %>, '<%= a.getTestName().replace("'", "\\'") %>', <%= a.getDuration() %>, <%= a.getTotalMarks() %>)" title="Edit">
										<i class="fas fa-edit"></i>
									</button>
									<form action="/api/admin/assessments/<%= a.getId() %>/delete" method="post" onsubmit="return confirm('Are you sure you want to delete this assessment?');" style="display: inline;">
							...
							%>
							<tr>
								<td colspan="6" style="text-align: center; color: var(--text-muted); padding: 25px;">No assessment test modules found. Use the form above to add one.</td>
							</tr>
							<%
							}
							%>
	...
	<div class="modal" id="editModal">
		<div class="modal-content">
			<div class="modal-header">
				<h3><i class="fas fa-edit"></i> Edit Assessment</h3>
				<button class="close-btn" onclick="closeEditModal()">&times;</button>
			</div>
			<form id="editForm" method="post">
				<div class="admin-form-group">
					<label for="editTestName">Test Title</label>
					<input type="text" id="editTestName" name="testName" required>
				</div>
				<div class="admin-form-group">
				...
				<div class="admin-form-group">
					...
					<input type="number" id="editTotalMarks" name="totalMarks" required>
				</div>
	...
	<script>
		...
		function openEditModal(id, title, duration, marks) {
			document.getElementById('editForm').action = '/api/admin/assessments/' + id + '/update';
			document.getElementById('editTestName').value = title;
		...
		document.addEventListener('DOMContentLoaded', function() {
			initTablePagination('assessmentsTable', 8);
		});
	</script>
</body>
</html>
